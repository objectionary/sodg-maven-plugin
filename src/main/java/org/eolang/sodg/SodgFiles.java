 ...
 *
 * @since 0.1
 * @todo #1:90min Refactor {@link SodgFiles} class to make it more readable.
...
final class SodgFiles {
     ...
     * SODG to Dot.
     */
    private static final Train<Shift> TO_DOT = new TrDot(SodgFiles.loggingLevel());

    /**
     * Graph modification right after it's generated from Xembly.
     */
    private static final Train<Shift> FINISH = new TrFinish(SodgFiles.loggingLevel());

    /**
     * The train that generates SODG.
     */
    private static final Train<Shift> TRAIN = new TrSodg(SodgFiles.loggingLevel());
    ...
    private int render(final Path xmir, final Path sodg) throws IOException {
        ...
        final XML after = new Xsline(this.measured(SodgFiles.TRAIN)).pass(before);
        final String instructions = new Xsline(this.measured(SodgFiles.TO_TEXT))
            .pass(after)
            .xpath("/text/text()")
            .get(0);
        if (Logger.isTraceEnabled(this)) {
        ...
        if (this.generateXemblyFiles) {
            final String xembly = new Xsline(this.measured(SodgFiles.TO_XEMBLY))
                .pass(after)
    ...
    private void makeGraph(final String xembly, final Path sodg) throws IOException {
        if (this.generateGraphFiles) {
            ...
            final XML graph = new Xsline(this.measured(SodgFiles.FINISH)).pass(
                new XMLDocument(
                    new Xembler(
                        new Directives()
    ...
    private void makeDot(final XML graph, final Path sodg) throws IOException {
        if (this.generateDotFiles) {
            final String dot = new Xsline(this.measured(SodgFiles.TO_DOT))
